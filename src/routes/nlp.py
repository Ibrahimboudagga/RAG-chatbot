...
async def index_project(request:Request, project_id:int, push_request: PushRequest):
    ...
    pbar = tqdm(total=total_chunks, desc="Indexing chunks", position = 0)
    ...
    while has_records:
        ...
        if not is_inserted:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseSignal.INSERT_INTO_VECTOR_DATBASE_ERROR.value
                }
            )
    ...
    return JSONResponse(
        content = { "signal" :ResponseSignal.INSERT_INTO_VECTOR_DATBASE_SUCCESS.value,
...
async def search_index(request: Request, project_id: int, serach_request: SearchRequest):
    ...
    results = await nlp_controller.search_vector_db_collection(
        project = project, text=serach_request.text, limit = serach_request.limit
    )
...
async def answer_question(request: Request, project_id: int, answer_request: SearchRequest):
    ...
    answer, full_prompt, chat_history = await nlp_controller.answer_rag_question(
        project = project, query = answer_request.text, limit = answer_request.limit
    )
